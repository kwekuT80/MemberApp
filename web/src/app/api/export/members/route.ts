...
export async function GET(request: NextRequest) {
  try {
    ...
    if (!members || members.length === 0) {
      return NextResponse.json({ error: 'No members found to export' }, { status: 404 });
    }

    // CSV Headers matching import format + extra columns for export
    const headers = [
      'surname', 'first_name', 'other_names', 'title',
      'occupation', 'phone', 'email', 'status',
      'rank', 'admission_date', 'last_assessment_year'
    ];

    // Get latest assessment year per member for export enrichment
    const { data: assessments }: any[] = await supabase
      .from('financial_assessments')
      .select('member_id, year')
      .order('year', { ascending: false });

    // Build year lookup map
    const memberLastYear = new Map<string, number>();
    if (assessments) {
      assessments.forEach((a: any) => {
        if (!memberLastYear.has(a.member_id)) {
          memberLastYear.set(a.member_id, a.year);
        }
      });
    }
      ...
      const row = headers.map(header => {
        switch (header) {
          case 'last_assessment_year':
            return String(memberLastYear.get(m.id) || '');
          default:
            return csvEscape((m as any)[header]);
        }
      });
    ...
    const filename = `ksji-members-export-${new Date().toISOString().split('T')[0]}.csv`;
