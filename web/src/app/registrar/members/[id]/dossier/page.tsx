...
export default function MemberDossierPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      // Query member with all related entities
      const { data, error } = await supabase
        .from('members')
        .select('*, spouse(*), children(*), dependents(*), emergency_contacts(*), positions(*), degrees(*), military(*), uniformed_rank_records(*)')
        .eq('id', id)
        .single();
      ...
      if (id) {
        try {
          ...
          // Fetch welfare disbursements for lifetime record
          const { data: disbData } = await supabase
            .from('welfare_disbursements')
            .select('*')
            .eq('member_id', id)
            .order('disbursement_date', { ascending: false });
          if (disbData) setDisbursements(disbData);
        } catch (e) {
  ...
  const safeDependents = Array.isArray(member.dependents) ? member.dependents : [];
  const safeEmergencyContacts = Array.isArray(member.emergency_contacts) ? member.emergency_contacts : [];
  const safeMilitary = Array.isArray(member.military) ? member.military : (member.military ? [member.military] : []);
      ...
      <div style={container}>
        ...
        <div id="dossier-print" style={paper}>
          ...
          <section style={section}>
            <h2 style={sectionLabel}>I. Personal & Membership Information</h2>
            <table style={table} className="dossier-table">
              <tbody>
                <tr>
                  <th style={th}>Full Name</th>
                  <td style={td} colSpan={3}><strong>{displayTitle} {firstName} {otherNames} {surname}</strong></td>
                </tr>
                <tr>
                ...
                <tr>
                  ...
                  <td style={td}>
                    <span style={{
                      ...
                      {member.status || (member.is_deceased ? 'Deceased' : 'Active')}
                    </span>
                  </td>
                </tr>
                <tr>
          ...
          {/* SECTION II: FAMILY & NEXT OF KIN */}
          <section style={section}>
            <h2 style={sectionLabel}>II. Family, Dependents & Next of Kin</h2>
            
            {/* Spouse Records */}
            {safeSpouse.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <h3 style={subSectionTitle}>Spouse Details</h3>
                <table style={table}>
                  <thead>
                    <tr>
                      <th style={tableH}>Spouse Name</th>
                      <th style={tableH}>Date of Birth</th>
                      <th style={tableH}>Nationality / Denomination</th>
                      <th style={tableH}>Ladies' Auxiliary Sisterhood</th>
                      <th style={tableH}>Parish / Notes</th>
                    </tr>
                  </thead>
                  <tbody>
                    {safeSpouse.map((s: any, idx: number) => {
                      const auxDetails = s.spouse_is_sister
                        ? `Yes — ${s.auxiliary_name || "Ladies' Auxiliary"}${s.auxiliary_number ? ` #${s.auxiliary_number}` : ''}`
                        : (s.spouse_is_sister === false ? 'No' : 'N/A');
                      return (
                        <tr key={idx}>
                          <td style={{ ...td, fontWeight: 700 }}>{s.spouse_name || 'N/A'}</td>
                          <td style={td}>{formatDisplayDate(s.spouse_dob)}</td>
                          <td style={td}>
                            {[s.spouse_nationality, s.spouse_denomination].filter(Boolean).join(' • ') || 'N/A'}
                          </td>
                          <td style={{ ...td, fontWeight: s.spouse_is_sister ? 700 : 400, color: s.spouse_is_sister ? '#065f46' : 'inherit' }}>
                            {auxDetails}
                          </td>
                          <td style={td}>
                            {[s.spouse_parish ? `Parish: ${s.spouse_parish}` : null, s.spouse_notes].filter(Boolean).join(' | ') || '—'}
                          </td>
                        </tr>
                      );
                    })}
            ...
            {safeChildren.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <h3 style={subSectionTitle}>Children ({safeChildren.length})</h3>
                <table style={table}>
                  <thead>
                    <tr>
                      <th style={tableH}>Child Name</th>
                      <th style={tableH}>Date of Birth</th>
                      <th style={tableH}>Place of Birth / Details</th>
                    </tr>
                  </thead>
                  <tbody>
                    {safeChildren.map((c: any, idx: number) => (
                      <tr key={idx}>
                        <td style={{ ...td, fontWeight: 700 }}>{c.child_name || 'N/A'}</td>
                        <td style={td}>{formatDisplayDate(c.birth_date)}</td>
                        <td style={td}>{c.birth_place || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {safeDependents.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <h3 style={subSectionTitle}>Dependents (Parents, In-Laws, etc.) ({safeDependents.length})</h3>
                <table style={table}>
                  <thead>
                    <tr>
                      <th style={tableH}>Dependent Name</th>
                      <th style={tableH}>Relationship</th>
                      <th style={tableH}>Date of Birth</th>
                    </tr>
                  </thead>
                  <tbody>
                    {safeDependents.map((d: any, idx: number) => (
                      <tr key={idx}>
                        <td style={{ ...td, fontWeight: 700 }}>{d.dependent_name || 'N/A'}</td>
                        <td style={td}>{d.relationship || 'Dependant'}</td>
                        <td style={td}>{formatDisplayDate(d.birth_date)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {safeEmergencyContacts.length > 0 && (
              <div style={{ marginBottom: 20 }}>
                <h3 style={subSectionTitle}>Emergency Contacts & Next of Kin ({safeEmergencyContacts.length})</h3>
                <table style={table}>
                  <thead>
                    <tr>
                      <th style={tableH}>Contact Name</th>
                      <th style={tableH}>Relationship</th>
                      <th style={tableH}>Primary Phone</th>
                      <th style={tableH}>Secondary Phone</th>
                    </tr>
                  </thead>
                  <tbody>
                    {safeEmergencyContacts.map((ec: any, idx: number) => (
                      <tr key={idx}>
                        <td style={{ ...td, fontWeight: 700 }}>{ec.contact_name || 'N/A'}</td>
                        <td style={td}>{ec.relationship || 'Next of Kin'}</td>
                        <td style={{ ...td, fontFamily: 'monospace', fontWeight: 600 }}>{ec.phone1 || '—'}</td>
                        <td style={{ ...td, fontFamily: 'monospace' }}>{ec.phone2 || '—'}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
            ...
            {safeSpouse.length === 0 && safeChildren.length === 0 && safeDependents.length === 0 && safeEmergencyContacts.length === 0 && (
              <table style={table}>
                <tbody>
                  <tr>
                    <td style={{ ...td, textAlign: 'center', color: '#718096' }}>No family, dependent, or next of kin records on file.</td>
                  </tr>
                </tbody>
              </table>
            )}
          ...
          {/* SECTION III: EXEMPLIFICATION HISTORY */}
          <section style={section}>
              ...
              <tbody>
                {sortedDegrees.map((d: any, idx: number) => (
                  <tr key={idx}>
                    ...
                    <td style={{ ...td, fontWeight: 700 }}>{d.degree_type || 'N/A'}</td>
          ...
          {/* SECTION IV: SERVICE HISTORY */}
          <section style={section}>
            <h2 style={sectionLabel}>IV. Record of Service & Positions Held</h2>
            <table style={table}>
              <thead>
                <tr>
              ...
              <tbody>
                {sortedPositions.map((p: any, idx: number) => (
                  <tr key={idx}>
                    ...
                    <td style={{ ...td, fontWeight: 700 }}>{p.position_title || 'N/A'}</td>
          ...
          {/* SECTION VI: DECEASED ARCHIVAL RECORD */}
          {(member.status === 'Deceased' || member.is_deceased) && (
            <section style={section}>
              <div style={{
                background: '#f8fafc',
                ...
                <h3 style={{ fontSize: 20, fontWeight: 900, color: '#0f172a', margin: '6px 0' }}>
                  🕯️ Deceased Member Record
                </h3>
                <p style={{ fontSize: 13, color: '#475569', margin: 0, lineHeight: 1.5 }}>
                  This record is permanently archived on the Commandery Roll of Honor for historical,
                  biographical, and service roll purposes. Active annual dues assessments, welfare
                  contribution obligations, and standing compliance evaluations are automatically exempt
                  and disengaged.
                </p>
              </div>
            </section>
          ...
          {/* SECTION VII: WELFARE BENEFITS & CLAIMS HISTORY */}
          {disbursements && disbursements.length > 0 && (
            <section style={section}>
              <h2 style={sectionLabel}>VI. Welfare Benefit Claims & Relief Disbursements</h2>
              <table style={table}>
                <thead>
                  <tr>
                    <th style={tableH}>Date Paid</th>
                    <th style={tableH}>Benefit Category</th>
                    <th style={tableH}>Payment Method</th>
                    <th style={tableH}>Notes / Reference</th>
                    <th style={{ ...tableH, textAlign: 'right' }}>Amount Paid</th>
                  </tr>
                </thead>
                <tbody>
                  {disbursements.map((d: any, idx: number) => (
                    <tr key={idx}>
                      <td style={td}>{formatDisplayDate(d.disbursement_date)}</td>
                      <td style={{ ...td, fontWeight: 700, color: '#0f172a' }}>{d.category_name}</td>
                      <td style={{ ...td, textTransform: 'capitalize' }}>{d.payment_method?.replace('_', ' ') || '—'}</td>
                      <td style={td}>{d.notes || d.reference_no || '—'}</td>
                      <td style={{ ...td, textAlign: 'right', fontWeight: 800, color: '#b91c1c', fontFamily: 'monospace' }}>
                        GH₵ {Number(d.amount || 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                      </td>
                    </tr>
                  ))}
                  <tr style={{ background: '#f8fafc' }}>
                    <td colSpan={4} style={{ ...td, fontWeight: 800, textAlign: 'right', color: '#1e293b' }}>
                      Total Welfare Benefits Received:
                    </td>
                    <td style={{ ...td, textAlign: 'right', fontWeight: 900, color: '#b91c1c', fontFamily: 'monospace' }}>
                      GH₵ {disbursements.reduce((sum, d) => sum + Number(d.amount || 0), 0).toLocaleString('en-US', { minimumFractionDigits: 2 })}
                    </td>
                  </tr>
                </tbody>
              </table>
          ...
          {/* SECTION VIII: STANDING COMPLIANCE EVALUATION (ACTIVE MEMBERS ONLY) */}
          {reportData && !member.is_deceased && member.status === 'Active' && (
            <section style={section}>
              <h2 style={sectionLabel}>VII. Standing & Compliance Evaluation</h2>
              <div style={{
                background: reportData.standing === 'In Good Standing' ? '#f0fdf4' : '#fff5f5',
                border: `1px solid ${reportData.standing === 'In Good Standing' ? '#bbf7d0' : '#fecaca'}`,
                borderRadius: '8px',
                padding: '20px'
              }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12 }}>
                  <div>
                    <span style={{ fontSize: 11, fontWeight: 800, color: '#64748b', textTransform: 'uppercase' }}>Official Overall Status</span>
                    <h3 style={{
                      fontSize: 18,
                      margin: '4px 0 0',
          ...
          {/* SECTION IX: FINANCIAL DUES SUMMARY (ACTIVE MEMBERS ONLY) */}
          {isFinancialRegistrar && reportData && !member.is_deceased && member.status === 'Active' && (
            <section style={section}>
              <h2 style={sectionLabel}>VIII. Financial Dues Ledger Summary</h2>
              <table style={table}>
                <tbody>
                  <tr>
          ...
          {/* SECTION X: WELFARE SCHEME SUMMARY (ACTIVE MEMBERS ONLY) */}
          {isWelfareTreasurer && reportData && !member.is_deceased && member.status === 'Active' && (
            <section style={section}>
              <h2 style={sectionLabel}>IX. Welfare Scheme Summary</h2>
              <table style={table}>
                <tbody>
                  <tr>
          ...
          {/* SECTION XI: VOLUNTARY RELIEF & SPECIAL APPEALS */}
          {(isFinancialRegistrar || isWelfareTreasurer) && reportData && (
            <section style={section}>
              <h2 style={sectionLabel}>X. Voluntary Relief & Special Appeals Record</h2>
              <table style={table}>
                <thead>
                  <tr>
                    <th style={tableH}>Date</th>
...
const subSectionTitle: React.CSSProperties = { fontSize: 14, color: '#10233f', fontWeight: 700, margin: '0 0 8px', textTransform: 'uppercase', letterSpacing: '0.5px' };
