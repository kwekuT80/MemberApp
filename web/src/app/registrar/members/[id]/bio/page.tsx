...
${sortedPositions.map(p => `• ${p.date_from ? p.date_from.split('-')[0] : '—'}: ${p.position_title} (${p.level || 'Local'} Level)`).join('\\par ')} \\par
\\par
\\sb200 \\b \\fs32 Exemplification History \\b0 \\fs28 \\par
${degrees.map(d => `• ${formatDate(d.degree_date)}: ${d.degree_type}`).join('\\par ')} \\par
\\par
\\qc \\ql \\par
\\par
\\par
