...
export default function MemberBioPage() {
  ...
  const exportToRTF = () => {
    ...
    const joined = member.date_joined ? new Date(member.date_joined).toLocaleDateString('en-GB', { month: 'long', year: 'numeric' }) : 'an unknown date';
    const spouse = member.spouse?.[0]?.spouse_name || 'N/A';
    
    const rtfContent = `{\\rtf1\\ansi\\deff0
{\\fonttbl{\\f0 Times New Roman;}}
\\f0\\fs24
\\qc \\b \\fs32 ${titleText.toUpperCase()} ${member.first_name.toUpperCase()} ${member.surname.toUpperCase()} \\b0 \\fs24 \\par
\\qc \\fs20 OFFICIAL SERVICE BIO & TESTIMONIAL \\fs24 \\par
\\ql \\par
\\b Introduction \\b0 \\par
${titleText} ${member.first_name} ${member.surname} joined the Knights of St. John International in ${joined}. Over the years, he has demonstrated unwavering commitment to the Order's values of Charity, Fraternity, and Service. \\par
\\par
\\b Family & Personal Life \\b0 \\par
...
\\par
\\b Service History \\b0 \\par
${sortedPositions.map(p => `• ${p.date_from ? p.date_from.split('-')[0] : '—'}: ${p.position_title} (${p.level || 'Local'} Level)`).join('\\par ')} \\par
\\par
\\b Exemplification \\b0 \\par
${degrees.map(d => `• ${d.degree_date ? d.degree_date.split('-')[0] : '—'}: ${d.degree_type}`).join('\\par ')} \\par
\\par
\\qc \\i Generated by KSJI Registrar Suite on ${new Date().toLocaleDateString()} \\i0
}`;
