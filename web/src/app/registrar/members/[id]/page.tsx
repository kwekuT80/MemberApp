...
export default async function RegistrarMemberDetailPage({ params }: { params: Promise<{ id: string }> }) { await requireRegistrar(); const { id } = await params; const member = await getMemberById(id); return <RegistrarShell title='Member Detail' subtitle='View and manage the selected member across all sections.'><div style={{ display:'grid', gap:18 }}><div style={{ display:'flex', gap:12, flexWrap:'wrap' }}><Link href='/registrar/members' style={{ textDecoration:'none', color:'#10233f', fontWeight:700 }}>Back to members</Link>{member?.id ? <><Link href={`/registrar/members/${member.id}/edit`} style={{ textDecoration:'none', color:'#10233f', fontWeight:700 }}>Edit main record</Link><Link href={`/registrar/members/${member.id}/bio`} style={{ textDecoration:'none', color:'#d4af37', fontWeight:700 }}>📜 View Service Bio</Link></> : null}</div>{member ? <><MemberSummaryCard member={member} editHref={`/registrar/members/${id}/edit`} showOwner /><RegistrarMemberActions memberId={member.id!} /></> : <EmptyState message='This member record could not be loaded.' />}</div></RegistrarShell>; }
