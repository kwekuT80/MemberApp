...
// Add compatibility fix for Gradle 8.8+ before the publishing block
def gradleCompatFix = { project ->
  try {
    // Try original approach first (works with most AGP versions)
    project.publishing.publications.release {
      from components.release
    }
  } catch (Throwable t) {
    // Fallback for Gradle 8.8+ compatibility
    project.publishing.publications.create('release', MavenPublication.class) {
      if (project.android.hasProperty('variants')) {
        def releaseVariant = project.android.variants.find { it.name == 'release' }
        if (releaseVariant != null) {
          from releaseVariant.outputs
        } else {
          // Last resort - use debug variant
...
project.afterEvaluate {
  publishing {
    publications {
      // Gradle 8.8+ compatibility - avoid direct access to components.release
      create('release', MavenPublication) {
        try {
          from components.release
        } catch (Exception e) {
          // Fallback for Gradle 8.8+ where components.release is not accessible
          def releaseVariant = project.android?.variants?.find { it.name == 'release' }
          if (releaseVariant != null && releaseVariant.outputs.isNotEmpty()) {
            from releaseVariant.outputs
          } else {
            // Last resort - use library outputs directly
            artifact project.tasks.findByName('bundleReleaseAar') ?:
