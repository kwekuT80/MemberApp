...
CREATE TABLE IF NOT EXISTS public.member_communications (
    ...
    content_preview TEXT,                   -- truncated preview of content sent
...
GRANT DELETE ON TABLE public.communication_requests TO authenticated;
...
CREATE POLICY "webhook_handler_can_insert_delivery_states"
    ...
    WITH CHECK (true);  -- Webhook function handles auth externally
END IF;
END $$;
...
CREATE TABLE IF NOT EXISTS public.registrar_queues (
    ...
    assigned_registrar_id UUID REFERENCES public.profiles(id),
...
GRANT INSERT, UPDATE, DELETE ON TABLE public.communication_templates TO authenticated;
